...
Now we not using general serializer ,we will be using that particular serializer for avro ,Avro serilaizer.

![Solution: Serializer asks Schema Registry to validate before publishing to Kafka](images/06-solution-validate-flow.png)

---
 but with every message we pass it is going to avro??
 is it adding one more hop..
...
# default is _schemas topic ,where we are storing all the schemas
# within this topic ,all schema version and schema is stored
...
```
We should have this in producer.This avsc  will become as java object 

Consider `"record"` as an Object. This `.avsc` will be used to generate a Java class named `Order` like below:
...
And it's not a normal POJO — it has additional functionality like `getSchema()`, which is used by the Serializer.This will have more additional methods.
...
we sending the java class generated as value.So Avro understand it is class generated by avsc so it will serialize it.
...
2. Look in Cache: Do I have Schema ID for this order Schema Hash? -> NO as it is first time

3. Http call (passes Subject: order-event-value, Schema) goes to Schema Registry
  ...
  compatibility is by default BACKWARD ,we will see it later 


   for value only we are serilaizing

4. Schema Registry responds: Schema Id: 1001.
5. Avro will Put into Cache: <Order Schema HASH, Schema ID>
6. Serialize( [magic byte][schema ID][payload])
...
O-1 C-1 5 55.0 -> only values these are
...
Not fields as we putting schemaID so it can get SChema from their.
```
...
# Currently using stringSerializer
